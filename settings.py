...
class VisualizerSettings(BaseModel):
    ...
    bar_color: str = Field(default="#FFFFFF", description="Hex color code for the bars")
    ...
    refresh_rate: int = Field(default=60, description="Target FPS for the visualizer")
    ...
    audio_device_index: int = Field(default=13, description="Index of the PyAudio device to use. Leave as -1 to use the default system loopback.")
    smoothing_factor: float = Field(default=0.6, description="Controls bar smoothness (0.0 to 0.99). Higher is smoother but slower.")
